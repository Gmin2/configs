...
-- mine added configs for tab splitting 

-- Quick Split Mappings
vim.keymap.set('n', '<leader>sv', '<cmd>vsplit<cr>', { desc = 'Split Vertically' })
vim.keymap.set('n', '<leader>sh', '<cmd>split<cr>', { desc = 'Split Horizontally' })
vim.keymap.set('n', '<leader>sc', '<cmd>close<cr>', { desc = 'Close Window' })
vim.keymap.set('n', '<leader>se', '<cmd>only<cr>', { desc = 'Maximize Current Window' })

vim.keymap.set('n', '<leader>st', '<cmd>vsplit | term<cr>', { desc = 'Split and Open Terminal' })

vim.keymap.set('n', '<leader>/', 'gcc', { desc = 'Comment line (n)' })
vim.keymap.set('v', '<leader>/', 'gc', { desc = 'Comment selection (v)' })
...
vim.diagnostic.config({
    virtual_text = false, -- This removes the long error text from the screen
    signs = true,         -- This keeps the W, E, H symbols in the gutter
    underline = false,     -- (Optional) Keeps the squiggly line under the error
...
-- Auto-Save feature:
-- When the cursor holds (no key pressed) for 'updatetime' (300ms, as recommended)
-- and the buffer is writable and modified, save the file silently.
...
-- Fix indentation for TypeScript, JavaScript, and JSON (use 2 spaces)
vim.api.nvim_create_autocmd("Filetype", {
...
require("lazy").setup({
	...
	-- commenting
	{
    	...
    	lazy = false,
    	opts = {}, -- The default options are excellent
	},
})
